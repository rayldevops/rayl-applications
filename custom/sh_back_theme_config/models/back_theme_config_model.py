...
class sh_back_theme_config_settings(models.Model):
    ...
    def search_primary_var_template(self):
        if self.env.ref('sh_backmate_theme.sh_assets_primary_variables').sudo():
            return self.env.ref('sh_backmate_theme.sh_assets_primary_variables').sudo().active
    
    def deactivate_primary_variable_scss(self):
        if self.env.ref('sh_backmate_theme.sh_assets_primary_variables').sudo():
            self.env.ref('sh_backmate_theme.sh_assets_primary_variables').sudo().write({'active':False})
        if self.env.ref('sh_backmate_theme.sh_assets_background_color_scss').sudo():
            self.env.ref('sh_backmate_theme.sh_assets_background_color_scss').sudo().write({'active':True})
        return True
    
    def activate_primary_variable_scss(self):
        if self.env.ref('sh_backmate_theme.sh_assets_primary_variables').sudo():
            self.env.ref('sh_backmate_theme.sh_assets_primary_variables').sudo().write({'active':True})
        if self.env.ref('sh_backmate_theme.sh_assets_background_color_scss').sudo():
            self.env.ref('sh_backmate_theme.sh_assets_background_color_scss').sudo().write({'active':False})
        return True
    ...
    def action_preview_theme_style(self):
        if self:
            ...
            if context and context.get('which_style','') == 'checkbox':
                img_src = "/sh_back_theme_config/static/src/img/preview/checkbox_preview.png"
                
            if context and context.get('which_style','') == 'radio':
                img_src = "/sh_back_theme_config/static/src/img/preview/radio_preview.png"
