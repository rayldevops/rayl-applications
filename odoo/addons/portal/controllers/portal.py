...
class CustomerPortal(Controller):
    ...
    def _prepare_portal_layout_values(self):
        ...
        if partner.is_affiliate:
            if partner.parent_aff.id:
                parent_partner = request.env['res.partner'].sudo().search([('id', '=', partner.parent_aff.id)])
        return {
    ...
    def home(self, **kw):
        ...
        aff_key = request.env.user.partner_id.res_affiliate_key
        user_email = request.env.user.partner_id.email
        aff_objs = request.env['affiliate.request'].sudo().search([('partner_id', '=', request.env.user.partner_id.id)])
        # if aff_obj:
        #     aff_obj.write({'parent_aff_key': aff_obj.website_parent_affiliate_key})
        if request.env.user.partner_id.is_affiliate:
            return request.render("portal.portal_my_home", values)
        elif aff_key:
            return http.request.redirect("/shop?affkey=%s" % aff_key)
            # return http.request.redirect("/shop?affkey=%s" % affiliate_obj.parent_aff_key)
