...
class WebsiteSale(http.Controller):
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        # affiliate_request = request.env['affiliate.request'].sudo().search(
        #     [('partner_id', '=', request.env.user.partner_id.id)])
        # if affiliate_request.bought_membership:
        #     values['membership_user'] = True
        # else:
        #     values['membership_user'] = False
        if category:
    ...
    def payment(self, **post):
        ...
        """
        order = request.website.sale_get_order()
        sale_partner = request.env['res.partner'].sudo().search([('id', '=', order.partner_id.id)], limit=1)
        if sale_partner:
            sale_partner.write({'affiliate_company_name': order.partner_id.company_name,
                         'company_name': None})
        redirection = self.checkout_redirection(order)
        if redirection:
            return redirection
    ...
    # @http.route(['/sale/payments'], type='http', auth="public", website=True)
    # def sale_payments(self, **post):
    #     _logger.info('-----------payment---------------')
    #     order_id = request.session.get('my_orders_history')
